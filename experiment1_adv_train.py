...
import time
from experiment1_std_train import exp_1_id
...
'''
print("-------------------------------Experiment1: adv_train:lr==3e-4,batch==16,train_layer==all layers------------------------------------")
'''Required parameters'''
# Path
...
for eps in [3,4,9]:
    ...
    expid = f"experiment1_adv=={eps}_train_lr==3e-4_batch==16_train_layer==all_layers_time=={expid_time}"
    exp_2_id = expid
    seed = 42
    ...
    '''
    print("-------------------------------Experiment1: adv_train:lr==1e-5,batch==16,train_layer==all layers------------------------------------")
    '''Required parameters'''
    # Path
    ds_path ="/content/data"
    OUT_DIR ="/content/drive/My Drive/logs"
    ...
    NUM_WORKERS = 2
    expid_time=datetime.now().strftime("%Y-%m-%d---%H:%M:%S")
    expid = f"experiment1_adv=={eps}_train_lr==1e-5_batch==16_train_layer==all_layers_time=={expid_time}"
    exp_3_id = expid
    seed = 42
